...
public class HUD extends PWindow {
	...
	public HUD(NewWindowManager parent, TurnManager tm, int x, int y,
			int width, int height) {
		super(parent, x, y, width, height);
		this.tm = tm;
		gp = new GradientPaint(0, 0, new Color(96, 116, 190), 0, height,
				new Color(0, 0, 54));
		ht = new GradientPaint(0, 0, new Color(169, 140, 86), 0, height,
				new Color(69, 40, 6));
		...
		centery = getHeight() / 2;
		events = new String[5];
		base = FileUtils.getImage("drawable-game/other/metaloid.png");
		base2 = flip(base);
		initButtons();
		update();
	}
	...
	private void initButtons() {
		addControl(missileB = new PButton(this, (getWidth() / 2) - 90,
				getHeight() - 60, 30, 30));
		addControl(bulletB = new PButton(this, (getWidth() / 2) - 30,
				getHeight() - 60, 30, 30));
		addControl(diplomacyB = new PButton(this, (getWidth() / 2) + 30,
				getHeight() - 60, 30, 30));
		addControl(moveB = new PButton(this, (getWidth() / 2) + 90,
				getHeight() - 60, 30, 30));
		addControl(nextMove = new PButton(this, "End Turn",
				(getWidth() / 2) - 60, getHeight() - 130, 150, 40));
		...
		missile.setImage(PImage.registerImage(FileUtils
				.getImage("drawable-game/Buttons/Missile.png")));
		bullet.setImage(PImage.registerImage(FileUtils
				.getImage("drawable-game/Buttons/Bullet.png")));
		diplomacy.setImage(PImage.registerImage(FileUtils
				.getImage("drawable-game/Buttons/Diplomacy.png")));
		move.setImage(PImage.registerImage(FileUtils
				.getImage("drawable-game/Buttons/Move.png")));
		...
		if (tm != null)
			if (tm.getTurn() != null)
				if (tm.getTurn().getPlayer() != null)
					g.drawString("" + tm.getTurn().getPlayer().name,
							(getWidth() / 3) + 25, 25);
		g.setFont(temp);
	}
	...
	public void update() {
		if (display != null || pinned) {
			...
			if (display != null) {
				...
				location = ("[X:" + display.getLocation().getCol() + " Y:"
						+ display.getLocation().getRow() + "]");
				if (display.getHandle() == 1) {
					MoveableEntity display = (MoveableEntity) this.display;
					if (display.isMovableTileBeingShown()) {
						display.toggleMovable();
					...
					movement = ("Movement Left: "
							+ (display.getMaxMovement() - display.getMoved())
							+ " out of " + display.getMaxMovement());
				}
			}
		} else {
			setVisible(false);
	...
	private void drawFrame(Graphics2D g, int x, int y, int width, int height) {
		...
		for (int x22 = x + 8; x22 < (x + width) - 8; x22 += 8) {
			g.drawLine(x22 + 4 + x - 50, y + height - 9, x22 + x - 50, y
					+ height);
		}
	...
	private boolean moveShip(int x, int y, boolean leftclick) {
		...
		if (leftclick
				&& GridHelper.canMoveTo(display.getManager(), display,
						display.getCurrentOrientation(), y, x,
						display.getWidth())) {
			if (display.isMovableTileBeingShown()) {
				display.toggleMovable();
			}
			display.moveTo(new Location(y, x));
			addEvent("Moving ship from (" + startr + "," + startc + ") to ("
					+ display.getLocation().getRow() + ","
					+ display.getLocation().getCol() + ")");
			...
			update();
			return true;
		} else if (GridHelper.canMoveTo(display.getManager(), display,
				display.getOppositeOrientation(), y, x, display.getWidth())) {
			if (display.isMovableTileBeingShown()) {
				display.toggleMovable();
			}
	...
	private boolean attackGuns(int x, int y, boolean leftclick) {
		...
		if (leftclick
				&& GridHelper.canAttackTo(display.getManager(), display, y, x)) {
			if (display.isAttackTileBeingShown()) {
				display.toggleAttackRange();
			}
			addEvent("Gunning ship from (" + startr + "," + startc + ") to ("
					+ y + "," + x + ")");
			display.useGuns();
			update();
			return true;
	...
	private boolean attackMissile(int x, int y, boolean leftclick) {
		...
		if (leftclick
			...
			addEvent("Tomahawk ship from (" + startr + "," + startc + ") to ("
					+ y + "," + x + ")");
			display.useMissiles();
			update();
			return true;
